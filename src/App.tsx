...
const BotAvatar = () => (
  <div className="flex-shrink-0 flex items-center justify-center w-10 h-10 rounded-full bg-white border border-gray-200 shadow-sm overflow-hidden p-[2px]">
    <img src={botAvatar} alt="Assistant IA" className="w-full h-full object-cover object-top rounded-full" />
  </div>
);
...
export default function App() {
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    // La sidebar est collée à droite : sa largeur est (largeur fenêtre - position souris X)
    const newWidth = window.innerWidth - e.clientX;
    // Bornes de sécurité : entre 320px et 60% de la largeur totale de l'écran
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 font-sans text-gray-900 print:h-auto print:bg-white">
      ...
      <header id="print-header" className="bg-white border-b border-gray-200 py-3 px-6 flex items-center justify-between shadow-sm z-10 print:shadow-none print:px-0 print:py-6 print:border-b-2">
        <div className="flex items-center">
          <div className="mr-4">
            <img
              src={profilePhoto}
              alt={tuteur.basics.name}
              className="w-16 h-16 rounded-full object-cover object-top border-2 border-indigo-100 shadow-sm cursor-zoom-in hover:scale-[1.1] hover:shadow-md transition-all duration-300"
              onClick={() => setFullscreenImage(profilePhoto)}
            />
          </div>
          <div>
            <h1 className="text-lg font-semibold">{tuteur.basics.name}</h1>
            <div className="flex flex-wrap items-center gap-4 mt-1 text-sm text-gray-600">
              <a
                href={`mailto:${tuteur.basics.email}`}
      ...
      {/* Corps Principal : Zone de discussion + Éventuel Volet Latéral d'Illustration */}
      <div className="flex-1 flex overflow-hidden relative print:block print:overflow-visible">
        
        {/* Section Chat (Discussion + Saisie) */}
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden print:block print:overflow-visible">
                        ...
                        <ReactMarkdown
                          components={{
                            ...
                            img: ({ node, ...props }) => (
                              <img
                                {...props}
                                className="max-w-full h-auto rounded-lg shadow-sm border border-gray-100 my-4 cursor-zoom-in hover:opacity-95 transition-opacity"
                                onClick={() => setFullscreenImage(props.src || null)}
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-0">
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Posez une question sur le cours (ex: Qu'est-ce qu'un skill ?)"
                    disabled={isLoading}
                    maxLength={500}
        ...
        {/* Barre de séparation réglable (sur grand écran uniquement) */}
        {fullscreenImage && (
          <div
            onMouseDown={startResizing}
            className="hidden lg:block w-1.5 hover:w-2 bg-gray-200 hover:bg-indigo-500 cursor-col-resize h-full transition-all duration-150 flex-shrink-0 relative z-20"
            title="Glissez horizontalement pour ajuster la taille"
          />
        )}

        {/* Volet Latéral Persistant d'Illustration (sur grand écran uniquement) */}
        {fullscreenImage && (
          <div
            style={{ width: `${sidebarWidth}px` }}
            className="hidden lg:flex bg-white border-l border-gray-200 flex-col h-full overflow-hidden shadow-lg animate-in slide-in-from-right duration-300 print:hidden flex-shrink-0"
          >
            {/* Header du volet */}
            <div className="p-4 border-b border-gray-200 flex items-center justify-end bg-gray-50 flex-shrink-0">
              <button
                onClick={() => setFullscreenImage(null)}
                className="p-1.5 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                title="Fermer l'illustration"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            {/* Corps du volet avec l'image (padding et marges optimisés pour agrandir l'image) */}
            <div className="flex-1 overflow-auto p-4 bg-gray-50 flex items-center justify-center">
              <img
                src={fullscreenImage}
                alt="Illustration agrandie"
                className="max-w-full max-h-full object-contain rounded-lg shadow-sm bg-white p-2"
              />
            </div>
          </div>
        )}
      ...
      {/* Modale Image Plein Écran (sur mobile/tablette uniquement, masquée sur lg) */}
      {fullscreenImage && (
        <div
          className="lg:hidden fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 cursor-zoom-out"
          onClick={() => setFullscreenImage(null)}
        >
          <button
          ...
          <img
            src={fullscreenImage}
            alt="Illustration agrandie"
            className="max-w-full max-h-[90vh] object-contain rounded-lg shadow-2xl cursor-default bg-white p-6"
